...
const CrystalVision: React.FC = () => {
  ...
  return (
    <div className="bg-[#0b0c10] text-white min-h-screen px-4 sm:px-6 md:px-12 py-12 sm:py-16">
      {/* ---------- HERO SECTION ---------- */}
      <header className="flex flex-col md:flex-row md:justify-between md:items-start mb-16 md:mb-20">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          custom={1}
          className="grid grid-cols-2 gap-4 mt-10 md:mt-0 md:ml-12"
        >
          {[
            ...
            <motion.div
              ...
              viewport={{ once: true }}
              custom={idx}
              className="bg-[#1a1c23] px-4 py-3 sm:px-5 sm:py-4 rounded-xl text-xs sm:text-sm hover:scale-105 transition-transform duration-300"
            >
      ...
      >
        <motion.div
          layout
          className="bg-secondary/30 p-1.5 sm:p-2 rounded-full border border-gray-700/50 flex gap-1 sm:gap-2 w-full max-w-md justify-between"
        >
          {tabs.map((tab, idx) => (
            <motion.button
              key={tab}
              ...
              whileTap={{ scale: 0.95 }}
              className={`flex-1 text-center px-2 sm:px-3 py-1.5 sm:py-2 rounded-full font-medium text-xs sm:text-sm md:text-base transition-all relative z-10 ${
                activeTab === tab
                  ? "text-white"
      ...
      {activeTab === "Logo Concepts" && (
        <motion.div
          ...
          variants={fadeUp}
          custom={3}
          className="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-12 sm:mb-16 auto-rows-[200px] sm:auto-rows-[260px] md:auto-rows-[320px]"
        >
          {logoImages.map((logo, idx) => (
            ...
            >
              <motion.img
                ...
                decoding="async" // ✅ Hint for browsers to optimize
              ...
              {hoveredImage === idx && (
                ...
                >
                  ...
                  <motion.button
                    whileTap={{ scale: 0.95 }}
                    className="bg-accent hover:bg-accent-light px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg font-medium transition-colors flex items-center gap-1.5 sm:gap-2 w-fit text-xs sm:text-sm"
                  >
      ...
      <motion.div
        ...
        viewport={{ once: true }}
        custom={4}
        className="bg-secondary/30 rounded-2xl p-6 sm:p-8 border border-gray-700/50 mb-12 sm:mb-16"
      >
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
          <div>
            <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3">3D & Rendering</h3>
            <p className="text-text-secondary text-xs sm:text-sm">Cinema 4D, Octane Render, KeyShot</p>
          </div>
          <div>
            <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3">Design & UI</h3>
            <p className="text-text-secondary text-xs sm:text-sm">Figma, Adobe Illustrator, Photoshop</p>
          </div>
          <div>
            <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3">Collaboration</h3>
      ...
      >
        <Link
          to="/works"
          className="flex items-center gap-1.5 sm:gap-2 text-text-secondary hover:text-accent transition-colors"
        >
      ...
      <motion.div
        ...
        viewport={{ once: true }}
        custom={6}
        className="text-center text-[10px] sm:text-xs text-gray-500 mt-10 sm:mt-12"
      >
